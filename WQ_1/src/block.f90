...
module block
  ...
  use material, only : block_material, init_material, init_material_from_file, &
                init_anelastic_properties, init_anelastic_Q_properties, &
 ...
 contains
     ...
     subroutine init_block(mesh_source, type_of_mesh,material_source,response,fd_type, order_fd, interpol, &
       use_topography, topo, B, problem, btp, block_comm,infile,id,ny,nz,debug)
    ...
    if (present(debug)) B%debug = debug
     ...
     if (interpol .eqv. .true.) call interpolatematerials(B,btp)
      ...
      ! Initialize exactly one attenuation model per response.
      ! All anelastic flags default to .false. (set in datatypes.f90; B is
      ! intent(out) here), so no explicit resets are needed. The init routine
      ! for each model sets its own flag(s).
      select case (trim(response))
      case ('low-pass', 'anelastic')
      ...
      case ('anelastic-Qf', 'frequency-Q-4M')
      ...
      case ('frequency-Q-8M')
        call init_anelastic_Qf8_properties(B%M, B%G, infile)
      end select
