...
program main
  ...
  use domain, only : domain_type,init_domain, close_domain, &
                     eval_mms, norm_fields, write_fault_output, in_block_comm
  ...
  do n = 1, D%nt
    ...
    if (is_master()) then 
     ...
     if (D%mms_vars%use_mms) then
        ...
        ! D%B(i) is valid only on ranks belonging to block i (and D%B(2)
        ! does not exist in 1-block mode), so print each block separately.
        if (in_block_comm(1)) then
           print *, 'MMS error in block 1: ',D%B(1)%sum*sqrt(D%B(1)%G%hq * D%B(1)%G%hr * D%B(1)%G%hs), &
                ' at time = ',D%t
        end if
        if (D%nblocks >= 2) then
           if (in_block_comm(2)) then
              print *, 'MMS error in block 2: ',D%B(2)%sum*sqrt(D%B(2)%G%hq * D%B(2)%G%hr * D%B(2)%G%hs), &
                   ' at time = ',D%t
           end if
        end if
